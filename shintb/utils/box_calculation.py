...
def draw_rect(I, r, c, thickness=1):
	if abs(sum(r)) < 100: # conditional to prevent min/max error
		cv2.rectangle(I, (int(r[0] * 300.0), int(r[1] * 300.0)),
					  (int((r[0] + max(r[2], 0)) * 300.0), int((r[1] + max(r[3], 0)) * 300.0)),
					  c, thickness)

# def draw_ann(I, r, text, color=(255, 0, 255), confidence=-1):
# 	draw_rect(I, r, color, 1)
# 	cv2.rectangle(I, (int(r[0] * 300.0), int(r[1] * 300.0 - 15)),
# 				  (int(r[0] * 300.0 + 100), int(r[1] * 300.0)),
# 				  color, -1)
...
# 	cv2.putText(I, text_, (int(r[0] * 300.0), int((r[1]) * 300.0)),
